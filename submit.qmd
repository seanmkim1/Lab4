...
2.  Preparing the data:
    ...
    met.first <- met[data.table::week(date) == 1]
...
6.  Use stat_summary to examine mean dew point and wind speed by region
    ...
    met_avg[!is.na(elev_cat)] %>% 
      ggplot()+
      stat_summary(aes(x = region, y = dew.point), fun.data = "mean_sd1", mult = 1, geom = "bar") + 
      stat_summary(aes(x = region, y = dew.point), fun.data = "mean_sd1", mult = 1, geom = "errorbar", width = 0.2)+
      labs(title = "mean dew point by region with stdev error bars",
           x = "region", 
           y = "mean dew point")+
      theme_bw()
    ```
